...
export async function runCodex(instructions,input,schema,{signal,spawnProcess=spawn}={}) {
 ...
 const abort=()=>{fail(new Error('Đã hủy hoặc hết thời gian chờ AI.'));proc?.kill();};
 try {
   ...
   createInterface({input:proc.stdout}).on('line',line=>{
     ...
     if(m.id!==undefined){const p=pending.get(m.id);if(p){pending.delete(m.id);m.error?p.reject(new Error('AI từ chối yêu cầu. Kiểm tra kết nối, đăng nhập và quyền truy cập.')):p.resolve(m.result);}return;}
     ...
     if(m.method==='turn/completed'){if(m.params?.turn?.status==='completed')resolveTurn(lastText);else fail(new Error('AI chưa hoàn tất lượt luận. Có thể đã hết hạn mức hoặc bị gián đoạn.'));}
   });
