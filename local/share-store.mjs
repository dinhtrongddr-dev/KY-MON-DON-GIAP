...
const MAX_SHARE_BYTES=256000,MAX_SHARE_FILES=1000,MAX_SHARE_STORAGE_BYTES=128*1024*1024,SHARE_TTL_MS=90*24*60*60*1000;
...
export async function loadShare(shareDir,id,{now=Date.now()}={}){
  if(!SHARE_ID.test(id))return null;
  const file=join(shareDir,id+'.json');
  try{
    const info=await stat(file);
    if(now-info.mtimeMs>SHARE_TTL_MS){await unlink(file).catch(()=>{});return null;}
    const record=JSON.parse(await readFile(file,'utf8'));return record?.schemaVersion===SHARE_SCHEMA?record:null;
  }catch{return null;}
}
