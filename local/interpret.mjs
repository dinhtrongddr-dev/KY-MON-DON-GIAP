...
export async function interpretReading(prepared,{runner=runAI,planRunner,budgetMs=READING_TIMEOUT_MS,signal}={}) {
  ...
  for(let attempt=0;attempt<2;attempt++) {
    ...
    const result=bindPlannedMetadata(await runner(instructionsFor(prepared.context),context,readingSchema(prepared.facts,prepared.context),{signal:combined}),prepared.context);
    combined.throwIfAborted();
