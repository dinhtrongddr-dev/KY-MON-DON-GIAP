...
export function validateReading(result,facts) {
 if(!result || !['reading','needs_clarification'].includes(result.status) || !TOPICS.some(t=>t.id===result.topic_id) || typeof result.summary!=='string' || !result.summary.trim()) throw new Error('AI trả kết quả chưa đúng cấu trúc. Hãy thử lại.');
