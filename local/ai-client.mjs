...
export const MODEL='astra → gemini';
...
export async function runAI(instructions,input,schema,{signal,codexRunner=runCodex}={}){
  const failures=[];
  for(const [index,route] of AI_ROUTES.entries()){
    signal?.throwIfAborted();
    try{
      const value=await codexRunner(instructions,input,schema,{signal,model:route.model,effort:route.effort});
  ...
  const error=new Error('Cả Astra và Gemini đều chưa hoàn tất lượt luận. Hãy thử lại sau.');
