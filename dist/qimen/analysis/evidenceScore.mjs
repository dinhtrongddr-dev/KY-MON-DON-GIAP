...
export function scoreEvidence(bundles,context,graph) {
  ...
  const ranked=bundles.map(b=>{
    ...
    const score=Number((questionRelevance+actorRelevance+3*goalRelevance+2*stageRelevance+0.4*structuralStrength+0.2*corroboration).toFixed(3));
    return {...b,relevance:{score,evidenceScore:score,questionRelevance,actorRelevance,goalRelevance,stageRelevance,structuralStrength,corroboration,modifierImportance,conflictPriority,
  ...
  const required=ranked.filter(b=>b.actorIds.some(id=>['self','event'].includes(id))||b.roles.some(r=>r.status==='user_supplied'));
