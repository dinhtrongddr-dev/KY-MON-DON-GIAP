...
export function analyzeBoard(board,{topic='general',actors={},selfPillar=board.pillars.day,questionContext=null,nianming={}}={}) {
  ...
  const palaces=board.palaces.filter(p=>p.number!==5).map(p=>({...p,conditions:palaceConditions(p),strength:seasonalStrength(board,p),
    ...
    stemPairs:p.heavenStems.map((s,i)=>({heaven:s,earth:p.earthStem,carried:i>0,relation:elementLink(s.element,p.earthStem.element),
      ...
      tomb:palaceConditions(p).stemTombs.includes(s.vi)}))}));
  ...
  return freezeData(validateAnalysis({schemaVersion:'QimenAnalysis/1',boardVersion:board.schemaVersion,
    ...
    coverage:{computed:['Nhật/Thời can và Giáp ẩn','KM-YONGSHEN-2.0 theo domain và thứ bậc Dụng Thần','KM-STRUCTURE-2.0: 81 Thập Can Khắc Ứng + Tứ hại + cách cục ưu tiên','KM-STRENGTH-2.0: Tinh/Môn/Can/Cung tách mô hình + Thập Nhị Trường Sinh','KM-ROLE-2.0: Chủ–Khách theo ngữ cảnh + Nội/Ngoại + agency + nhiều vai','KM-KEYING-3.0: 64 Môn×Môn + 72 Môn×Kỳ/Nghi + 24 Tam Kỳ đáo cung + 108 chỉ mục Cửu Tinh trị thời + Hòa/Nghĩa/Bức/Chế','KM-FORMATION-3.0: Tam Trá + Ngũ Giả + Cửu Độn + Thiên Tam Môn + Địa Tứ Hộ','KM-DIRECTION-4.0: Địa Tư Môn + Đình Đình/Bạch Gian + Thiên Mã/Thiên Cương + Tam Thắng/Ngũ Bất Kích','KM-NIANMING-1.0: Niên Mệnh tự nhập làm corroborator, không thay Nhật can/Dụng Thần','64 quan hệ cung có chiều','Không/Mã','hai chiều Môn–Cung','kích hình từng can','Nhập mộ 9 can hiện bàn: Ất→2/Bính→6/Đinh→8/Mậu→6/Kỷ→8/Canh→8/Tân→4/Nhâm→4/Quý→2','các lớp phản/phục ngâm','Ứng kỳ v3: nhịp Nội/Ngoại + Không/Kích Hình/Mã/Mộ + xung/hợp chi ẩn Lục Nghi','mâu thuẫn'],
      unsupported:['Diễn giải hiện đại đầy đủ cho 108 Cửu Tinh trị thời (Phase 9 chỉ index classical_context_only)','Phương vị quân sự/ẩn độn ngoài KM-DIRECTION-4.0 chưa triển khai nếu chưa có profile nguồn riêng','Full 本命行年 cổ điển (Hành Niên, Nam/Nữ thuận nghịch, Ngũ Hổ độn, Nạp Âm) chưa nằm trong KM-NIANMING-1.0','Phản/Phục ngâm tự định ngày; Tinh–Môn sinh/khắc chưa ép thành một can-chi đơn trị','Phi Bàn / Cửu Thần','Tâm ý hoặc quyền quyết định thực tế khi chưa được xác nhận']}}));
}
