...
Mỗi related_consideration ghi claim_ids hỗ trợ nếu có; không có claim thì để []. source_quote phải trích nguyên văn câu hỏi cho source=explicit; entailed/check_only để source_quote rỗng. Không thêm kết luận deterministic mới cho một khía cạnh.
...
export function deliberationSchema(context){
  ...
  return obj({
    semantic_frame:obj({
      ...
      related_considerations:arr(obj({label:str,why_relevant:str,source:choice(['explicit','entailed','check_only']),source_quote:str,claim_ids:arr(choice(claimIds))})),
      scope_guard:arr(str)
    }),
