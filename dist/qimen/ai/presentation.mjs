...
export function buildPresentationProfile(context) {
  ...
  const profile={
    ...
    showTiming:hasRealTiming,
    showAlternative:true,
    minActions:2,
    minClaimGroups:3,
    tabs:[],
    labels:{situation:'Căn cứ quyết định',bottleneck:'Điểm mấu chốt',alternative:'Khi nào cần đổi cách hiểu?',actions:'Bước tiếp theo',timing:'Ứng kỳ'},
  ...
  if(mode==='prediction'){
    ...
    profile.tabs=[['quick','Kết quả'],...(profile.showDevelopment?[['story','Diễn biến']]:[]),['actions','Điều cần làm'],['technical','Căn cứ Kỳ Môn'],...(profile.showTiming?[['timing','Ứng kỳ']]:[])];
    profile.labels={situation:'Cụm tượng quyết định',bottleneck:'Điều kiện làm thay đổi kết quả',alternative:'Nhánh kết quả khác',actions:'Điều cần làm',timing:'Ứng kỳ'};
    profile.reasoningGoal='Trả lời kết quả đang hỏi trước; tách dấu hiệu, điều kiện chuyển và kết quả cuối. Chỉ dựng diễn biến khi câu hỏi thực sự cần hoặc người dùng chọn Luận sâu. Chỉ gọi là ứng kỳ khi engine có mốc được phép.';
  } else if(mode==='strategy'){
    profile.showTiming=false;
