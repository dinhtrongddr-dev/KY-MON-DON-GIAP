...
export function translateToRealWorld(bundle,context) {
  const d=domainSemantics(context.domain),core=DOORS[bundle.symbols.door.id],capacity=STARS[bundle.symbols.star.id];
  const role=bundle.actorIds.includes('self')?'self':bundle.actorIds.includes('authority')?'authority':bundle.actorIds.includes('event')?'event':bundle.actorIds[0];
  const social=bundle.symbols.deity.id==='chief'?(role==='self'?'self_agency':role==='authority'?'authority':'central_coordination'):DEITIES[bundle.symbols.deity.id];
  const socialMeaning=social==='self_agency'?'phần chủ động và quyền xử lý của chính người hỏi':social==='central_coordination'?'khâu giữ nhịp và tổ chức sự việc':SOCIAL[social];
  const conditional=bundle.conflicts.some(c=>c.code==='opening_unrealized');
  const objective={access:d.opportunity,contact:role==='self'?'khả năng giữ nhịp và chuẩn bị của người hỏi':'khả năng trao đổi và làm rõ nhu cầu',growth:context.domain==='finance'?'khả năng tạo ra khoản thu, chưa đồng nghĩa tiền thực nhận':d.value,active_friction:'khâu cần tác động, sửa hoặc cạnh tranh',contained_work:'phần việc cần làm rõ bên trong',presentation:'thông tin, hồ sơ hoặc cách trình bày',closure:'khâu kết thúc, giữ lại hoặc xử lý việc cũ',communication_pressure:'thông tin gây sức ép hoặc cần làm rõ'}[core];
  ...
  return {mechanism,domain:context.domain,objective,capacity,social,
    stageAsked:context.outcomeTarget?.stageAsked,actorRole:role,
    interaction:{opening:objective,means:CAPACITY[capacity],counterpartCondition:socialMeaning,modifiers},
    roleContexts:bundle.roles.map(r=>({role:r.id,meaning:r.meaning,status:r.status})),
    manifestation:{status:'hypothesis_to_verify',concepts:[objective,CAPACITY[capacity],socialMeaning],vocabulary:context.vocabulary.slice(0,12)},
    ...
    action:{verb:conditional?'verify_then_activate':core==='communication_pressure'?'clarify_before_commit':core==='closure'?'check_status_before_reopen':'test_next_step',
      focus:CAPACITY[capacity],counterpartCheck:socialMeaning,object:objective,
