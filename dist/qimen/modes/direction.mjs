...
export function direction(a,{action='general'}={}) {
  action=normalizeAction(action);
  const ranking=rankCandidates(a.palaces.map(p=>({id:`direction_${p.number}`,palace:p.number,label:`${p.direction} · ${p.vi} ${p.number}`,...actionMetrics(p,action)})));
  ...
  ]),{action,ranking,convention:RANK_CONVENTION,origin:null});
}
