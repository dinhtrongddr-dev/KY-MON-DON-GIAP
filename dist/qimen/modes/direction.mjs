...
export function direction(a,{action='general',direction=null}={}) {
  ...
  const ranking=direction?.origin&&direction?.kind?rankCandidates(a.palaces.map(p=>({id:`direction_${p.number}`,palace:p.number,label:`${p.direction} · ${p.vi} ${p.number}`,...actionMetrics(p,action)}))):[];
