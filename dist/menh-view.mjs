...
import {formatParts,mountAiTechnicalToggle,renderSurfaceReading} from './reading-format.mjs';
...
function focusParagraph(text){
  ...
  for(const part of formatParts(String(text||''),{automatic:false})){
    ...
    // Historical text remains intact; no inferred evidence spans.
...
export function renderMenhAi(container,reading){
  if(reading?.narrativeVersion){renderSurfaceReading(container,reading);return;}
  container.replaceChildren();
