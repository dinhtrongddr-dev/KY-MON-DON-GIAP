...
test('Mệnh long-form reading visually emphasizes fast-scan translated takeaways without HTML injection',()=>{
  const view=readFileSync(new URL('../dist/menh-view.mjs',import.meta.url),'utf8');
  assert.doesNotMatch(view,/FOCUS_PATTERNS/);assert.match(view,/formatParts/);assert.match(view,/automatic:false/);assert.match(view,/menh-ai-focus/);assert.doesNotMatch(view,/ai-technical-prefix/);assert.match(view,/renderSurfaceReading/);assert.match(view,/mountAiTechnicalToggle/);assert.match(view,/semanticCardNode/);
