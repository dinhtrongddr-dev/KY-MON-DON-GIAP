...
Bản 16.1.0 dùng TG-CB-5.1 / protocol 5, giữ nguyên lõi lập bàn. Thêm kiểm tra dữ kiện trong văn, nguồn/giới hạn của ánh xạ, bài ngắn theo câu hỏi, chữ đậm an toàn, đầu vào phương hướng và phần dữ kiện dự phòng sau một lượt sửa không đạt. Nguồn chính là GitHub, host Cloudflare Pages tại kymon.pp.ua. Đã xác nhận tài khoản có GPT-6 Astra/high, nhưng lượt luận thực bị chặn do hết hạn mức Codex; chưa nghiệm thu chất lượng bài Astra. Chi tiết và giới hạn kiểm thử tại `docs/releases/AI-REQUIREMENTS-2026-09-15.md`.
...
6. Khởi động lại đúng server, nhập mã mới trên website rồi bấm Kiểm tra kết nối. `/api/status` qua Worker phải báo `rules: TG-CB-5.1`, `protocol: 5`. Kiểm tra trạng thái thành công chưa chứng minh model đã chạy.
